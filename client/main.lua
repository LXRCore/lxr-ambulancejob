...
local function DoLimbAlert()
    if not isDead and not InLaststand then
        if #injured > 0 then
            ...
            exports['lxr-core']:Notify(9, limbDamageMsg, 5000, 0, 'blips', 'blip_radius_search', 'COLOR_WHITE')
        end
    end
end
...
local function DoBleedAlert()
    if not isDead and tonumber(isBleeding) > 0 then
        exports['lxr-core']:Notify(9, Lang:t('info.bleed_alert', {bleedstate = Config.BleedingStates[tonumber(isBleeding)].label}), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
    end
end
...
local function ResetAll()
    ...
    TriggerServerEvent("LXRCore:Server:SetMetaData", "hunger", 100)
    TriggerServerEvent("LXRCore:Server:SetMetaData", "thirst", 100)
end
    ...
    for k, v in pairs(WeaponDamageList) do
        if Citizen.InvokeNative(0xDCF06D0CDFF68424, ped, GetHashKey(k), 0) then --HasPedBeenDamagedByWeapon(
            ...
            if not IsInDamageList(k) then
                exports['lxr-core']:Notify(9, Lang:t('info.status')..': '..v, 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
...
RegisterNetEvent('ambulance:client:promptCheckin', function()
    if doctorCount >= Config.MinimalDoctors then
        ...
        exports['lxr-core']:Progressbar("hospital_checkin", Lang:t('progress.checking_in'), 2000, false, true, {
            ...
            if bedId then
                ...
                exports['lxr-core']:Notify(9, Lang:t('error.beds_taken'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
            end
        end, function() -- Cancel
...
RegisterNetEvent('hospital:client:ambulanceAlert', function(coords, text)
    exports['lxr-core']:Notify(9, text, "ambulance")
    local transG = 250
...
RegisterNetEvent('hospital:client:Revive', function()
    ...
    emsNotified = false
    exports['lxr-core']:Notify(9, Lang:t('info.healthy'), 5000, 0, 'blips', 'blip_radius_search', 'COLOR_WHITE')
end)
...
RegisterNetEvent('hospital:client:HealInjuries', function(type)
    ...
    TriggerServerEvent("hospital:server:RestoreWeaponDamage")
    exports['lxr-core']:Notify(9, Lang:t('success.wounds_healed'), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
end)
...
RegisterNetEvent('hospital:client:SendToBed', function(id, data, isRevive)
    ...
    CreateThread(function ()
        Wait(5)
        if isRevive then
            exports['lxr-core']:Notify(9, Lang:t('success.being_helped'), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
...
RegisterNetEvent('hospital:client:RespawnAtHospital', function()
    ...
    if exports['lxr-policejob']:IsHandcuffed() then
...
RegisterNetEvent('hospital:client:SendBillEmail', function(amount)
    SetTimeout(math.random(2500, 4000), function()
        ...
        if exports['lxr-core']:GetPlayerData().charinfo.gender == 1 then
            gender = Lang:t('info.mrs')
        end
        local charinfo = exports['lxr-core']:GetPlayerData().charinfo
        TriggerServerEvent('lxr-phone:server:sendNewMail', {
