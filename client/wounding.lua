--[[
    ...
    🐺 LXR Ambulance Job — Client Wounding System
...
local function DoBleedAlert()
    if not isDead and tonumber(isBleeding) > 0 then
        exports['lxr-core']:Notify(9, "You are "..Config.BleedingStates[tonumber(isBleeding)].label, 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
    end
end
...
RegisterNetEvent('hospital:client:UseIfaks', function()
    ...
    exports['lxr-core']:Progressbar("use_bandage", Lang:t('progress.ifaks'), 3000, false, true, {
        ...
        TriggerServerEvent("LXRCore:Server:RemoveItem", "ifaks", 1)
...
RegisterNetEvent('hospital:client:UseBandage', function()
    local ped = PlayerPedId()
    exports['lxr-core']:Progressbar("use_bandage", Lang:t('progress.bandage'), 4000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
        ...
        TriggerServerEvent("LXRCore:Server:RemoveItem", "bandage", 1)
...
RegisterNetEvent('hospital:client:UsePainkillers', function()
    local ped = PlayerPedId()
    exports['lxr-core']:Progressbar("use_bandage", Lang:t('progress.painkillers'), 3000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
        ...
        TriggerServerEvent("LXRCore:Server:RemoveItem", "painkillers", 1)
            ...
            if bleedTickTimer >= Config.BleedTickRate and not isInHospitalBed then
                if not isDead and not InLaststand then
                    if isBleeding > 0 then
                        ...
                        TriggerServerEvent("evidence:server:CreateBloodDrop", exports['lxr-core']:GetPlayerData().citizenid, exports['lxr-core']:GetPlayerData().metadata["bloodtype"], coords)
