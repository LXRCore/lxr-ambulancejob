--[[
    ...
    🐺 LXR Ambulance Job — Client Last Stand System
        ...
        CreateThread(function()
            while InLaststand do
                ...
                elseif LaststandTime - 1 <= 0 then
                    exports['lxr-core']:Notify(9, Lang:t('error.bled_out'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
                    SetLaststand(false)
                    ...
                    TriggerServerEvent("lxr-log:server:CreateLog", "death", Lang:t('logs.death_log_title', {playername = GetPlayerName(-1), playerid = GetPlayerServerId(player)}), "red", Lang:t('logs.death_log_message', {killername = killerName, playername = GetPlayerName(player), weaponlabel = weaponLabel, weaponname = weaponName}))
                    deathTime = 0
                    OnDeath()
                    DeathTimer()
                end
...
RegisterNetEvent('hospital:client:UseFirstAid', function()
    if not isEscorting then
        ...
        exports['lxr-core']:Notify(9, Lang:t('error.impossible'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
    end
end)
...
RegisterNetEvent('hospital:client:HelpPerson', function(targetId)
    ...
    exports['lxr-core']:Progressbar("hospital_revive", Lang:t('progress.revive'), math.random(30000, 60000), false, true, {
